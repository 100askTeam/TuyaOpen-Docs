...
# 0: 未启动/停止，1: 正在运行
SYSTEM_STATE = 1
# Socket 配置 (请修改为 Tuya 设备的 IP)
SERVER_IP = '127.0.0.1'
SERVER_PORT = 8888
TRACKING_ENABLED = True # 涂鸦下发的跟随开关 (PROP_BOOL)
MANUAL_MODE = False      # 是否处于遥控模式
CURRENT_CMD = "stop"     # 当前遥控指令
REMOTE_SMOOTH_ALPHA = 0.25  # 遥控平滑系数：0.1-0.2 之间。越小越丝滑，但延迟感略增。
# --------------------

# 人体姿态检测，使用 YOLOv8-Pose 模型
...
CENTER_X_MIN = 0.3  # 画面宽度的 30%
CENTER_X_MAX = 0.7  # 画面宽度的 70%
CENTER_Y_MIN = 0.3  # 画面高度的 30%
CENTER_Y_MAX = 0.7  # 画面高度的 70%
...
def move_arm_based_on_pose(current_action, pan_center, shoulder_center, elbow_center, wrist_roll_center, ex, ey, box_width, frame_width):
    ...
    target_action = current_action.clone()
        ...
        # --- 引入控制死区 (Dead Zone) ---
    # 如果误差在±DEAD_ZONE 像素以内，就认为已经对准，不进行移动，防止抖动。
...
def search_for_person(robot, joint_names, search_pan, search_dir, pan_center, SEARCH_PAN_RANGE, SEARCH_STEP_DEG, target_action):
    ...
    - target_action: The target action with the new shoulder_pan position.
...
def socket_client_thread(host, port):
    ...
    while True:
        ...
        try:
            ...
            print(f"[Socket] Connected to Tuya device!")
            while True:
                data = client_socket.recv(1024)
                if not data:
                    print("[Socket] Server closed connection")
                    break

                # 处理粘包：依靠 C 端发送的 \n 分割消息
                msgs = data.decode('utf-8').strip().split('\n')
                for one_msg in msgs:
                    if not one_msg.strip():
                        continue
                    try:
                        ...
                        if 'dpid' in msg and msg['dpid'] == 1:
                            ...
                            if TRACKING_ENABLED:
                                SYSTEM_STATE = 1
                                MANUAL_MODE = False
                                print("[Socket] Tracking ENABLED")
                            else:
                                print("[Socket] Tracking DISABLED")

                        # 处理遥控指令
                        elif msg.get('type') == 'control':
                            cmd = msg.get('command')
                            # 无论 TRACKING_ENABLED 状态如何，都进入手动模式
                            MANUAL_MODE = True
                            SYSTEM_STATE = 1
                            CURRENT_CMD = cmd
                            print(f"[Socket] Received Command: {cmd}")
                    except json.JSONDecodeError as e:
                        print(f"[Socket] JSON decode error: {e}, raw: {one_msg}")
                    except Exception as e:
                        print(f"[Socket] Error processing msg: {e}")
        except ConnectionRefusedError:
            print(f"[Socket] Connection refused, retrying in 3s...")
            time.sleep(3)
        except Exception as e:
            print(f"[Socket] Connection error: {e}, retrying in 3s...")
            time.sleep(3)
        finally:
            try:
                client_socket.close()
...
def main():
    ...
    # 1. 启动 Socket 线程
    print(f"[Socket] 启动 Socket 客户端线程，目标：{SERVER_IP}:{SERVER_PORT}")
    ...
    step = 0
    last_box = None
    smoothed_action = None  # 初始化平滑动作变量
    print("系统已启动，等待接收 Tuya 指令...")
    try:
        while True:
            # 计算两帧之间的时间差 dt
            ...
            if MANUAL_MODE:
                ...
                if CURRENT_CMD == "exit":
                    ...
                    # 整体向前：台灯向前移动
                    # shoulder_lift 增加，elbow_flex 减小，wrist_flex 保持水平
                    lift_before = target_action[0, idx['shoulder_lift.pos']].item()
                    elbow_before = target_action[0, idx['elbow_flex.pos']].item()
                    target_action[0, idx['shoulder_lift.pos']] += step * 0.8
                    target_action[0, idx['elbow_flex.pos']] -= step * 1.2
                    # wrist_flex 保持不动，让台灯保持水平
                    lift_after = target_action[0, idx['shoulder_lift.pos']].item()
                    elbow_after = target_action[0, idx['elbow_flex.pos']].item()
                    print(f"[forward] shoulder_lift: {lift_before:.1f} -> {lift_after:.1f}, elbow_flex: {elbow_before:.1f} -> {elbow_after:.1f}")
                    ...
                    lift_after = target_action[0, idx['shoulder_lift.pos']].item()
                    elbow_after = target_action[0, idx['elbow_flex.pos']].item()
                    print(f"[backward] shoulder_lift: {lift_before:.1f} -> {lift_after:.1f}, elbow_flex: {elbow_before:.1f} -> {elbow_after:.1f}")
                elif CURRENT_CMD == "turn_left":
                ...
                target_action[0, idx['wrist_flex.pos']] = torch.clamp(target_action[0, idx['wrist_flex.pos']], -80, 80)

                # --- 指令平滑滤波器 (EMA) ---
                # 让实际发送的指令追赶目标位置，消除卡顿感
                smoothed_action_prev_lift = smoothed_action[0, idx['shoulder_lift.pos']].item()
                smoothed_action_prev_elbow = smoothed_action[0, idx['elbow_flex.pos']].item()
                smoothed_action = REMOTE_SMOOTH_ALPHA * target_action + (1 - REMOTE_SMOOTH_ALPHA) * smoothed_action
                smoothed_action_new_lift = smoothed_action[0, idx['shoulder_lift.pos']].item()
                smoothed_action_new_elbow = smoothed_action[0, idx['elbow_flex.pos']].item()
                print(f"[smoothed] shoulder_lift: {smoothed_action_prev_lift:.1f} -> {smoothed_action_new_lift:.1f}, elbow_flex: {smoothed_action_prev_elbow:.1f} -> {smoothed_action_new_elbow:.1f}")
            ...
            # 场景 B: 跟随模式 (TRACKING_ENABLED=True)
                ...
                for x in results[:3]:
                    ...
                    if x.shape[2] == 20:
                        stride, layer_idx = 32, 20*4*20*2 + 20*2*20*2
                    elif x.shape[2] == 40:
